...
function init() {
  ...
  function addFieldToIdentity(value, key){
    chrome.storage.local.get('identities', ({identities=[]})=>{
      if(!identities.length){ showNotification('No identities saved'); return; }
      const opts = identities.map(i=>`<option value="${i.id}">${i.identityName||'Unnamed'}</option>`).join('');
      const modal = createStyledModal('Add to Identity', `
        <select id="idSel" style="width:100%;padding:6px;border-radius:6px;background:#0b1220;color:#e2e8f0;border:1px solid #334155;">${opts}</select>
        <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:12px;">
          <button id="idCancel" style="background:#ff6b6b;border:none;color:#fff;padding:6px 12px;border-radius:6px;cursor:pointer;">Cancel</button>
          <button id="idSave" style="background:#39ff14;border:none;color:#000;padding:6px 12px;border-radius:6px;cursor:pointer;">Save</button>
        </div>
      `);
      setTimeout(()=>{
        document.getElementById('idSave').addEventListener('click',()=>{
          const chosen=document.getElementById('idSel').value;
          const idx=identities.findIndex(i=>i.id===chosen);
          ...
          showNotification('Saved to identity');
        });
        document.getElementById('idCancel').addEventListener('click',()=>{ const m=document.getElementById('zepra-styled-modal'); if(m) m.remove(); });
      },100);
    });
  }
    ...
    function render(user){
      ...
      `;
        ...
        btn.addEventListener('click',()=>{
          ...
          addFieldToIdentity(fields[idx].value, fields[idx].key);
        });
      });
