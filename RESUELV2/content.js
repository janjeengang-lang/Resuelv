...
function init() {
  ...
  function showIPQualificationModal(data){
    ...
    let listed = false;
    if(Array.isArray(blacklists)){ listed = blacklists.length > 0; }
    else if(typeof blacklists === 'object'){ listed = Object.values(blacklists).some(v => v && v !== '0' && v !== 'false'); }
    const status = risk < 30
      ? { msg: 'You have qualified', color: '#39ff14' }
      : risk <= 50
        ? { msg: 'Maybe you have qualified', color: '#ffe600' }
        : { msg: "Sorry, you haven't qualified. Please change proxy or VPN", color: '#ff4444' };
    const needle = risk * 3.6 - 90;

    const html = `
      <style>
        .ipq-wrap{display:flex;flex-direction:column;align-items:center;gap:20px;color:#e2e8f0;}
        .ipq-gauge{position:relative;width:180px;height:180px;border-radius:50%;background:conic-gradient(from -90deg,#39ff14 0deg 108deg,#ffe600 108deg 180deg,#ff4444 180deg 360deg);box-shadow:0 0 20px rgba(57,255,20,0.3),0 0 20px rgba(255,230,0,0.3);}
        .ipq-needle{position:absolute;width:4px;height:50%;background:#fff;top:50%;left:50%;transform-origin:bottom center;transform:rotate(${needle}deg);transition:transform 0.5s ease-out;}
        .ipq-gauge::before{content:'';position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:20px;height:20px;background:#000;border-radius:50%;z-index:2;border:2px solid #39ff14;}
        .ipq-center{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-weight:bold;z-index:3;color:#e2e8f0;}
        .ipq-status{font-size:18px;font-weight:bold;text-align:center;}
        .ipq-details{width:100%;text-align:left;line-height:1.6;}
        .ipq-details strong{color:#39ff14;}
      </style>
      <div class="ipq-wrap">
        <div class="ipq-gauge">
          <div class="ipq-needle"></div>
          <div class="ipq-center">Risk: ${risk}</div>
        </div>
        <div class="ipq-status" style="color:${status.color};">${status.msg}</div>
        <div class="ipq-details">
          <div><strong>IP:</strong> ${ip} - ${flag} ${city ? city+', ' : ''}${cc}</div>
          <div><strong>ISP:</strong> ${isp || 'Unknown'}</div>
          <div><strong>Blacklist Status:</strong> <span style="color:${listed ? '#ff4444' : '#39ff14'};">${listed ? 'Listed' : 'Clean'}</span></div>
        </div>
      </div>`;
