  ...
  function showIPQualificationModal(data){
    ...
    const html = `
      <style>
        ...
        .ipq-gauge{position:relative;width:200px;height:100px;overflow:hidden;}
        .ipq-arc{position:absolute;bottom:0;left:0;width:200px;height:200px;border-radius:50%;background:conic-gradient(#39ff14 0deg 54deg,#ffe600 54deg 90deg,#ff4444 90deg 180deg,transparent 180deg 360deg);box-shadow:0 0 20px rgba(57,255,20,0.3),0 0 20px rgba(255,230,0,0.3);}
        .ipq-needle{position:absolute;bottom:0;left:50%;width:4px;height:90px;background:#fff;transform-origin:bottom center;transform:rotate(-90deg);transition:transform 1s ease-out;}
        .ipq-center{position:absolute;bottom:0;left:50%;transform:translate(-50%,50%);padding:2px 6px;background:#000;border:2px solid #39ff14;border-radius:4px;font-weight:bold;}
      ...
      <div class="ipq-wrap">
        ...
        <div class="ipq-gauge">
          <div class="ipq-arc"></div>
          <div class="ipq-needle" id="ipqNeedle"></div>
          <div class="ipq-center">${risk}</div>
        </div>
    ...
    const modal = createStyledModal('IP Qualification', html);
    const needleEl = modal.querySelector('#ipqNeedle');
    setTimeout(() => { needleEl.style.transform = `rotate(${needleAngle}deg)`; }, 50);
  }
