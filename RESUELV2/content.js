...
function init() {
  ...
  async function showRealAddressModal(){
    ...
    function parse(text){
      return {
        a1: /Address\s*1:\s*(.*)/i.exec(text)?.[1]?.trim() || '',
        a2: /Address\s*2:\s*(.*)/i.exec(text)?.[1]?.trim() || '',
        zip: /Zip\s*Code:\s*(.*)/i.exec(text)?.[1]?.trim() || ''
      };
    }
    ...
    try {
      ...
      if (modal) {
        ...
        modal.querySelector('.btn-write-all').addEventListener('click', async () => {
          closeModal();
          const parts = parseAnswers(answer);
  ...
  function parseAnswers(text){
    return text.split(/\s*\d+[\.)]\s*/).filter(Boolean).map(s=>s.trim());
  }
