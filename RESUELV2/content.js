...
function init() {
  ...
  function addFieldToIdentity(btn, value, key){
  ...
  function toggleIdentityPanel(){
    ...
    panel.id = 'zepra-id-panel';
    panel.style.cssText = 'position:fixed;top:0;right:0;width:220px;height:100%;background:#111;color:#e2e8f0;z-index:2147483647;padding:10px;box-shadow:0 0 15px #39ff14;overflow:auto;';
    const close = document.createElement('button');
    close.textContent='×'; close.style.cssText='position:absolute;top:5px;right:5px;background:none;border:none;color:#fff;font-size:16px;cursor:pointer;';
    ...
    fields.forEach(k=>{
      if(activeIdentity[k]){
        ...
        row.style.cssText='margin:8px 0;font-size:12px;';
        row.innerHTML=`<strong>${k}:</strong> <span>${activeIdentity[k]}</span> <button data-copy="${k}" style="margin-left:6px;background:#22c55e;border:none;color:#000;padding:2px 6px;border-radius:4px;cursor:pointer;font-size:10px;">Copy</button>`;
        panel.appendChild(row);
      }
    });
    panel.querySelectorAll('[data-copy]').forEach(btn=>{
      btn.addEventListener('click',()=>{ navigator.clipboard.writeText(activeIdentity[btn.dataset.copy]||''); });
    });
  ...
  function watchForms(){
    const check = ()=>{
      ...
      if(forms.length){
        ...
        bar.querySelector('#zepra-fill').onclick=()=>{ fillForm(forms[0]); bar.remove(); };
        bar.querySelector('#zepra-dismiss').onclick=()=>bar.remove();
      }
    };
