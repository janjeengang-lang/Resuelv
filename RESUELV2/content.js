...
async function setupYouTubeDubbing() {
  ...
  console.log('Zepra Dubbing: Waiting for YouTube player controls...');

  const inject = () => {
    const player = document.querySelector('.html5-video-player');
    const controls = player && player.querySelector('.ytp-right-controls');
    if (!controls) return false;
    if (controls.querySelector('#zepra-dub-bubble')) return true;

    console.log('Zepra Dubbing: Target found! Injecting button.');
    const bubble = document.createElement('button');
    bubble.id = 'zepra-dub-bubble';
    bubble.className = 'ytp-button';
    bubble.textContent = '🔊';
    ...
    controls.insertBefore(bubble, settingsBtn || controls.firstChild);
    ...
    bubble.addEventListener('mousedown', e => {
      ...
      // Reparent to player for free movement
  ...
  try {
    ...
    if (tracks?.length) {
      ...
      if (url) {
        ...
        const segs = xmlToSegments(xml);
        if (segs.length) return segs;
      }
    }
  } catch (e) {
    console.warn('ytInitialPlayerResponse parsing failed', e);
  }

  // Method 2: scrape transcript panel if open
...
function xmlToSegments(xmlStr) {
  const xml = new DOMParser().parseFromString(xmlStr, 'text/xml');
  return Array.from(xml.getElementsByTagName('text')).map(n => ({
