...
async function setupYouTubeDubbing() {
  ...
  if (document.getElementById('zepra-dub-btn')) return;
  const container = document.querySelector('#below #top-level-buttons-computed') || document.querySelector('#below');
  if (!container) return;
  const wrap = document.createElement('div');
  wrap.id = 'zepra-dub-controls';
  ...
  btn.textContent = 'Dubbing by Zepra';
  ...
  btn.addEventListener('click', () => startDubbing(select.value));
}

async function startDubbing(lang) {
  try {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      let audio = await cacheGet(key);
      if (!audio) {
        const translated = await translateSegment(seg.text, lang);
        ...
        audio = tts.audio;
        await cacheSet(key, audio);
      }
      audioSegs.push({ start: seg.start, audio });
    }
...
async function fetchTranscript() {
  ...
  if (!tracks?.length) throw new Error('No transcript');
