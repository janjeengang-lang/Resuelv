...
function render(){
  ...
  for(const id of identities){
    ...
    card.innerHTML = `
      ...
      <img src="${id.profilePictureUrl||'icons/zepra.svg'}" alt="pf">
      <div class="name">${id.identityName||'No name'}</div>
      <div>${id.country||''}</div>
      <div class="actions">
        <button class="btn act">Activate</button>
        <button class="btn edit">Edit</button>
        <button class="btn del">Delete</button>
      </div>
      <label class="toggle"><input type="checkbox" class="proxyToggle" ${id.useProxy?'checked':''}> Use Proxy</label>
    `;
    card.querySelector('.act').addEventListener('click',()=>activate(id.id));
