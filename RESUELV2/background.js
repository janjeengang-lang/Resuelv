      ...
      switch (message.type) {
          ...
          try {
            await chrome.proxy.settings.set({
              value: {
                ...
                rules: {
                  singleProxy: {
                    scheme: (message.proxy.proxyType || 'http').toLowerCase(),
                    host: message.proxy.proxyIp,
                    port: parseInt(message.proxy.proxyPort, 10) || 0
                  }
                }
              },
              scope: 'regular'
            ...
            try {
              ...
              sendResponse({ ok: false, error: verifyErr.message || 'Verification failed' });
            }
          } catch (e) {
        ...
        case 'TEST_PROXY': {
          try {
            ...
            const info = await getPublicIP(true);
            sendResponse({ ok: true, info });
          } catch (e) {
            sendResponse({ ok: false, error: e.message });
          } finally {
