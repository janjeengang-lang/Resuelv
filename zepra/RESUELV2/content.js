...
function init() {
  ...
  const SURVEY_AGENT_DEFAULT_GOAL = 'Complete the requested task carefully.';
  ...
  function appendSurveyAgentAgentMessage(text, meta = 'status') {
    ...
    if (lastAgent && lastAgent.text === trimmed && lastAgent.meta === meta) {
      return;
    }
    appendSurveyAgentChatMessage({ role: 'agent', text: trimmed, meta });
  }
  ...
  function sendSurveyAgentUserMessage(text) {
    ...
    if (session.running) {
      appendSurveyAgentChatAck('Understood! Working that into the plan.', 'ack-running');
    } else {
      appendSurveyAgentChatAck('Great! I\'ll kick things off for you.', 'ack-idle');
    }
  }
  ...
  function surveyAgentStatusDescriptor(session) {
    ...
    const counts = `${steps === 1 ? '1 step' : `${steps} steps`} • ${plans === 1 ? '1 plan' : `${plans} plans`}`;

    if (session.running && session.stopRequested) {
      return { label: 'Stopping…', badge: 'is-warning', meta: counts };
    }
    if (session.running) {
      switch (session.status) {
        case 'scanning':
          return { label: 'Scanning page', badge: 'is-running', meta: counts };
        case 'planning':
          return { label: 'Planning next move', badge: 'is-running', meta: counts };
        case 'executing':
          return { label: 'Executing step', badge: 'is-running', meta: counts };
        case 'review':
          return { label: 'Reviewing progress', badge: 'is-running', meta: counts };
        case 'rate-limit': {
          const waitText = session.rateLimitBackoffMs
            ? `${Math.max(1, Math.ceil(session.rateLimitBackoffMs / 1000))}s cooldown`
            : 'Cooling down';
          return { label: 'Cooling down', badge: 'is-warning', meta: `${counts} • ${waitText}` };
        }
        case 'loop-guard':
          return { label: 'Needs attention', badge: 'is-warning', meta: `${counts} • waiting for guidance` };
        default:
          return { label: 'Running', badge: 'is-running', meta: counts };
      }
    }

    switch (session.status) {
      case 'done':
        return { label: 'Completed', badge: 'is-success', meta: counts };
      case 'aborted':
        return { label: 'Aborted', badge: 'is-warning', meta: counts };
      case 'error':
        return { label: 'Error', badge: 'is-error', meta: counts };
      case 'stopped':
        return { label: 'Stopped', badge: 'is-warning', meta: counts };
      default:
        return { label: 'Idle', badge: 'is-idle', meta: 'Waiting for instructions' };
    }
  }
  ...
  function updateSurveyAgentUiState() {
    ...
    if (elements.runToggle) {
      elements.runToggle.textContent = session.running ? 'Stop' : 'Start';
      elements.runToggle.classList.toggle('is-stop', Boolean(session.running));
      elements.runToggle.disabled = Boolean(session.running && session.stopRequested);
    }
  ...
  function formatSurveyAgentResult(result) {
    ...
    if (details.skipped) {
      if (details.text) return `Already "${truncateForLog(details.text)}"`;
      if (details.value) return `Already "${truncateForLog(details.value)}"`;
    ...
    if (details.delta !== undefined) return `Scroll by ${Math.round(details.delta)}px`;
  ...
  function finalizeSurveyAgentSession(session, status, message) {
    ...
    const summaryMessage =
      ...
      (status === 'done'
        ...
        : 'Agent is idle.');
    if (summaryMessage) {
      appendSurveyAgentAgentMessage(summaryMessage, 'status');
    }
    if (status === 'done') {
      const payload = {
  ...
  function openSurveyAgentPanel() {
    ...
    const settings = getSurveyAgentSettings();
    const identityCheckedAttr = settings.useIdentityAnswers === false ? '' : 'checked';
    const deliberateCheckedAttr = settings.deliberateMode ? 'checked' : '';
        ...
        <header class="nano-agent-header">
          <div class="nano-brand">
            ...
            <div class="nano-heading">
              <div class="nano-title">Survey Agent</div>
              <div class="nano-status-line">
                <span class="nano-status-badge is-idle">Idle</span>
                <span class="nano-status-text">Waiting for instructions</span>
              </div>
            </div>
          </div>
          <div class="nano-header-actions">
            <button type="button" class="nano-run-toggle">Start</button>
            <button type="button" class="survey-agent-close" aria-label="Close survey agent panel">&times;</button>
          </div>
        </header>
        <section class="nano-controls">
          <label class="nano-toggle">
            <input type="checkbox" class="nano-toggle-identity" ${identityCheckedAttr} />
            <span>Use the active identity profile whenever a field matches.</span>
          </label>
          <label class="nano-toggle">
            <input type="checkbox" class="nano-toggle-deliberate" ${deliberateCheckedAttr} />
            <span>Deliberate mode: slow down, double-check answers, and avoid repeating actions.</span>
          </label>
        </section>
        <section class="nano-thread">
          <div class="nano-placeholder">Say hello and I\'ll get the survey moving.</div>
          <div class="nano-messages"></div>
        </section>
        <form class="nano-compose">
          <textarea class="nano-input" rows="3" placeholder="What can I help you with?"></textarea>
          <footer>
            <span class="nano-hint">Shift+Enter for new line</span>
            <button type="submit" class="nano-send" disabled>Send</button>
          </footer>
        </form>
      </div>
    `;
    ...
    ui.elements = {
      ...
      runToggle: panel.querySelector('.nano-run-toggle'),
      identityToggle: panel.querySelector('.nano-toggle-identity'),
      deliberateToggle: panel.querySelector('.nano-toggle-deliberate'),
    ...
    if (ui.elements.identityToggle) {
      ui.elements.identityToggle.addEventListener('change', (event) => {
        const enabled = Boolean(event.target?.checked);
        updateSurveyAgentSetting('useIdentityAnswers', enabled);
        const sessionState = getSurveyAgentSession();
        if (sessionState.running) {
          sessionState.useIdentityAnswers = enabled;
          appendSurveyAgentAgentMessage(
            enabled
              ? 'I will use the active identity profile for matching fields.'
              : 'Identity autofill is now disabled for this run.',
            'status'
          );
        }
      });
    ...
    if (ui.elements.deliberateToggle) {
      ui.elements.deliberateToggle.addEventListener('change', (event) => {
        const enabled = Boolean(event.target?.checked);
        updateSurveyAgentSetting('deliberateMode', enabled);
        const sessionState = getSurveyAgentSession();
        if (sessionState.running) {
          sessionState.deliberateMode = enabled;
          appendSurveyAgentAgentMessage(
            enabled
              ? 'Switching to deliberate pacing so I can double-check each step.'
              : 'Returning to regular pacing.',
            'status'
          );
        }
      });
    ...
    if (ui.elements.runToggle) {
      ui.elements.runToggle.addEventListener('click', () => {
        const sessionState = getSurveyAgentSession();
        if (sessionState.running) {
          stopSurveyAgentRun();
          return;
        ...
        if (input && input.value.trim()) {
          ...
          sendSurveyAgentUserMessage(instructions);
          input.value = '';
          if (ui.elements.chatSend) {
            ui.elements.chatSend.disabled = true;
          }
        } else {
          const chat = Array.isArray(sessionState.chat) ? sessionState.chat : [];
          for (let idx = chat.length - 1; idx >= 0; idx -= 1) {
  ...
  function startSurveyAgentRun(options = {}) {
    ...
    if (session.instructions) {
      appendSurveyAgentAgentMessage(`On it! I'll work on: ${session.instructions}`, 'status');
    } else {
      appendSurveyAgentAgentMessage('Got it! Let me scan the page and queue the first move.', 'status');
    }
    if (session.useIdentityAnswers) {
      appendSurveyAgentAgentMessage('Using the active identity profile for matching fields.', 'status');
    }
    if (session.deliberateMode) {
      appendSurveyAgentAgentMessage('Deliberate mode enabled — I\'ll pace the steps to avoid rate limits and missed updates.', 'status');
    }
  ...
  function stopSurveyAgentRun(reason = 'Stop requested by user') {
    ...
    appendSurveyAgentAgentMessage('Okay, stopping after the current step.', 'status');
  }
  ...
  async function runSurveyAgentLoop(session) {
    try {
      while (session.running && !session.stopRequested) {
        ...
        updateSurveyAgentUiState();
        updateSurveyAgentPhase(session, 'scanning', 'Scanning the page for new questions…', 'status');
        ...
        updateSurveyAgentUiState();
        updateSurveyAgentPhase(session, 'planning', 'Thinking through the next best move…', 'plan');
        ...
        if (!planResponse || !planResponse.ok) {
          ...
          if (isRateLimit) {
            ...
            updateSurveyAgentPhase(
              session,
              'rate-limit',
              `Cerebras is rate limiting us, waiting ${waitSeconds}s before trying again.`,
              'warning'
            );
          ...
          appendSurveyAgentAgentMessage(`I couldn't plan the next step: ${errorMessage}. Retrying…`, 'error');
        ...
        if (!plan.step) {
          ...
          appendSurveyAgentAgentMessage('I did not get a clear next action. Let me rescan and try again.', 'plan');
          if (session.errorCount >= SURVEY_AGENT_MAX_PLAN_ERRORS) {
        ...
        const signature = getSurveyAgentStepSignature(plan.step);
        if (signature) {
        ...
        if (session.repeatPlanCount >= SURVEY_AGENT_PLAN_REPEAT_LIMIT) {
          appendSurveyAgentLog({
            level: 'warning',
            label: 'Loop guard',
            message: 'Planner repeated the same step',
            detail: planMessage,
          });
          ...
          updateSurveyAgentPhase(
            session,
            'loop-guard',
            "I'm seeing the same instruction without progress. Could you guide me on the next move?",
            'warning'
          );
          session.history.push({
            ...
            result: {
              ...
              details: { locator: plan.step?.locator || {}, reason: 'repeat_plan_limit' },
            },
          });
        ...
        if (session.repeatedSuccessCount >= SURVEY_AGENT_REPEAT_STUCK_LIMIT) {
          appendSurveyAgentLog({
            level: 'warning',
            label: 'Loop guard',
            message: 'Action repeated with no progress',
            detail: planMessage,
          });
          session.status = 'loop-guard';
          updateSurveyAgentUiState();
          updateSurveyAgentPhase(
            session,
            'loop-guard',
            "I already completed that step but the page didn't change. Could you double-check or guide me?",
            'warning'
          );
          session.history.push({
            step: { ...plan.step },
            result: {
              ...
              details: { locator: plan.step?.locator || {}, reason: 'repeat_no_progress' },
            },
          });
          if (session.history.length > SURVEY_AGENT_HISTORY_LIMIT) {
            session.history = session.history.slice(-SURVEY_AGENT_HISTORY_LIMIT);
        ...
        updateSurveyAgentUiState();
        updateSurveyAgentPhase(session, 'executing', `Executing: ${planMessage}`, 'status');
        ...
        session.lastExecutedSignature = signature || '';
        ...
        if (!normalizedResult.ok) {
          ...
          appendSurveyAgentAgentMessage(`Action failed: ${failureDetail}. I'll adjust and try again.`, 'error');
        }

        session.status = 'review';
        updateSurveyAgentUiState();
        updateSurveyAgentPhase(session, 'review', 'Double-checking the page before planning again.', 'status');
  ...
  async function executeSurveyAgentAction(action = {}) {
    ...
    let resolved = null;
    if (type === 'scroll') {
      if (action.mode && action.mode !== 'element') {
    ...
    if (type === 'scroll' && (!resolved || !resolved.doc)) {
      resolved = {
        element: resolved?.element || null,
        doc: document,
        frameElements: resolved?.frameElements || [],
        frameChain: resolved?.frameChain || [],
