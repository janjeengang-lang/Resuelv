...
function init() {
  ...
  function loadDomBuilderModule() {
    ...
    const module =
      globalThis.__zepraBuildDomTreeModule ||
      (typeof globalThis.__zepraBuildDomTree === 'function'
        ? { buildDomTree: globalThis.__zepraBuildDomTree }
        : typeof globalThis.buildDomTree === 'function'
        ? { buildDomTree: globalThis.buildDomTree }
        : null);
    if (module && typeof module.buildDomTree === 'function') {
      domBuilderModulePromise = Promise.resolve(module);
    ...
    const runtimeUrl =
      ...
      typeof chrome.runtime.getURL === 'function'
        ? chrome.runtime.getURL('src/dom/buildDomTree.js')
        : null;

    if (!runtimeUrl) {
      return Promise.reject(new Error('DOM builder unavailable'));
    }

    domBuilderModulePromise = import(runtimeUrl)
      .then((mod) => {
        const builder =
          mod?.buildDomTree ||
          mod?.default ||
        ...
        if (typeof builder !== 'function') {
          throw new Error('DOM builder missing export');
        }
        const loadedModule = { buildDomTree: builder };
        try {
          if (!globalThis.__zepraBuildDomTreeModule) {
            globalThis.__zepraBuildDomTreeModule = loadedModule;
          }
          if (!globalThis.__zepraBuildDomTree) {
            globalThis.__zepraBuildDomTree = builder;
          }
          if (!globalThis.buildDomTree) {
            globalThis.buildDomTree = builder;
          }
        } catch (err) {
          // Ignore global assignment failures (e.g., strict CSP frames)
        }
        return loadedModule;
      })
      .catch((err) => {
        ...
        throw new Error(`Failed to load DOM builder: ${message}`);
      });
  ...
  function ensureSurveyAgentStyles() {
    ...
    style.textContent = `
      ...
      #zepra-survey-agent-panel.is-open {
        ...
        pointer-events: auto;
        box-shadow: -28px 0 48px rgba(15, 23, 42, 0.55);
      }
      ...
      #zepra-survey-agent-panel .survey-agent-shell {
        ...
        width: 100%;
        height: 100%;
        background: linear-gradient(200deg, rgba(8, 12, 24, 0.94), rgba(2, 6, 14, 0.97));
        color: #e2e8f0;
      ...
      #zepra-survey-agent-panel .survey-agent-status {
        ...
        background: linear-gradient(130deg, rgba(15, 23, 42, 0.85), rgba(11, 18, 34, 0.78));
      }
      ...
      #zepra-survey-agent-panel .survey-agent-status-dot.is-running {
        ...
        box-shadow: 0 0 0 2px rgba(56, 189, 248, 0.22), 0 0 16px rgba(34, 211, 238, 0.45);
        animation: surveyAgentPulse 1.4s ease-in-out infinite;
      }

      #zepra-survey-agent-panel .survey-agent-status-dot.is-success {
        background: linear-gradient(135deg, #22c55e, #4ade80);
        box-shadow: 0 0 0 2px rgba(34, 197, 94, 0.24), 0 0 16px rgba(74, 222, 128, 0.45);
      }

      #zepra-survey-agent-panel .survey-agent-status-dot.is-warning {
        background: linear-gradient(135deg, #facc15, #f97316);
        box-shadow: 0 0 0 2px rgba(250, 204, 21, 0.24), 0 0 16px rgba(249, 115, 22, 0.45);
      }

      #zepra-survey-agent-panel .survey-agent-status-dot.is-error {
        background: linear-gradient(135deg, #f87171, #ef4444);
        box-shadow: 0 0 0 2px rgba(248, 113, 113, 0.24), 0 0 16px rgba(239, 68, 68, 0.45);
      }
      ...
      #zepra-survey-agent-panel .survey-agent-btn.start {
        background: linear-gradient(135deg, rgba(56, 189, 248, 0.22), rgba(14, 165, 233, 0.3));
      ...
      #zepra-survey-agent-panel .survey-agent-btn.start:hover:not(:disabled) {
        ...
        box-shadow: 0 16px 34px -18px rgba(56, 189, 248, 0.9);
      }
      ...
      #zepra-survey-agent-panel .survey-agent-btn.stop:hover:not(:disabled) {
        transform: translateY(-1px);
        box-shadow: 0 16px 32px -20px rgba(248, 113, 113, 0.7);
      }
      ...
      #zepra-survey-agent-panel .survey-agent-btn:focus-visible {
        outline: none;
      ...
      #zepra-survey-agent-panel .survey-agent-chat {
        ...
        background: linear-gradient(150deg, rgba(15, 23, 42, 0.8), rgba(11, 18, 32, 0.92));
      }
      ...
      #zepra-survey-agent-panel .survey-agent-chat-bubble {
        ...
        line-height: 1.45;
        max-width: 100%;
        box-shadow: 0 12px 28px -22px rgba(59, 130, 246, 0.6);
      }

      #zepra-survey-agent-panel .survey-agent-chat-message.is-user .survey-agent-chat-bubble {
        background: linear-gradient(135deg, rgba(59, 130, 246, 0.32), rgba(37, 99, 235, 0.42));
      ...
      #zepra-survey-agent-panel .survey-agent-chat-send {
        ...
        padding: 0.7rem 1rem;
        background: linear-gradient(135deg, rgba(59, 130, 246, 0.28), rgba(14, 165, 233, 0.36));
        color: #e0f2fe;
        font-weight: 600;
        letter-spacing: 0.05em;
        text-transform: uppercase;
      ...
      #zepra-survey-agent-panel .survey-agent-chat-send:hover:not(:disabled) {
        transform: translateY(-1px);
        box-shadow: 0 16px 32px -20px rgba(59, 130, 246, 0.8);
      }
  ...
  function ensureSurveyAgentChatGreeting() {
    ...
    appendSurveyAgentChatMessage({
      role: 'agent',
      text: 'Ready when you are. Type a command here and press Send or Start to let me handle the survey.',
      meta: 'greeting',
    });
  }
      ...
      <div class="survey-agent-panel">
        ...
        <section class="survey-agent-chat">
          <div class="survey-agent-chat-body">
            <div class="survey-agent-chat-empty">Share a command for the agent and press Send or Start.</div>
            <div class="survey-agent-chat-thread"></div>
          </div>
          <form class="survey-agent-chat-compose">
            <textarea class="survey-agent-chat-input" rows="2" placeholder="Describe what you want the agent to do…"></textarea>
            <button type="submit" class="survey-agent-chat-send" disabled>Send</button>
          </form>
        </section>
      </div>
    `;
