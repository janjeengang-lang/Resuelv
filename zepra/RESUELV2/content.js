  ...
  async function runSurveyAgentLoop(session) {
    try {
      while (session.running && !session.stopRequested) {
        ...
        const planMessage = formatSurveyAgentAction(plan.step) || 'لا توجد خطوة قابلة للتنفيذ';
        const planDetails = [];
        ...
        updateSurveyAgentUiState();
        updateSurveyAgentPhase(session, 'executing', `تنفيذ الخطوة: ${planMessage}`, 'status');
